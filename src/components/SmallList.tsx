...
export default function SmallList({ title, items, showThumb = true }: SmallListProps) {
      ...
      <ul className="space-y-4">
        {items.map((it) => (
          <li key={it.id} className="flex flex-col gap-2">
            ...
            {it.url ? (
              <a href={it.url} target="_blank" rel="noopener" className="font-semibold hover:text-blue-600 transition">
                {it.title}
              </a>
            ) : (
